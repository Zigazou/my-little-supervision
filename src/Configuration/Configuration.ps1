...
function Assert-ConfigurationKeys {
  ...
  foreach ($key in $Data.Keys) {
    if ($key -notin $Allowed) {
      throw "${Context}: unsupported key '$key'."
    }
  }
}
...
function Import-MonitorConfiguration {
  ...
  Assert-ConfigurationKeys $data $configurationKeys 'Configuration'
  if (
    -not $data.ContainsKey('ConfigurationVersion') -or
    $data.ConfigurationVersion -isnot [int] -or
    $data.ConfigurationVersion -ne 1
  ) {
    throw 'ConfigurationVersion must be 1.'
  }

  if (
    -not $data.ContainsKey('Checks') -or
    $data.Checks -isnot [array]
  ) {
    throw 'Checks must be an array.'
  }

  if ($data.Checks.Count -gt 1000) {
    throw 'Checks may contain at most 1000 entries.'
  }

  $language = 'en-US'
  if ($data.ContainsKey('Language')) {
    if ($data.Language -notin @('en-US', 'fr-FR')) {
      throw 'Language must be en-US or fr-FR.'
    }
  ...
  $checks = foreach ($entry in $data.Checks) {
    if ($entry -isnot [hashtable]) {
      throw 'Each check must be a hashtable.'
    }
    ...
    Assert-ConfigurationKeys $entry $configurationKeys 'Check'

    foreach ($key in @('Name', 'Type')) {
      if (
        -not $entry.ContainsKey($key) -or
        $entry[$key] -isnot [string] -or
        [string]::IsNullOrWhiteSpace($entry[$key])
      ) {
        throw "Each check requires a non-empty $key string."
      }
    }

    if ($names.ContainsKey($entry.Name)) {
      throw 'Check names must be unique (case insensitive).'
    }
    ...
    if ($entry.Type -notin $entryTypes) {
      throw 'Type must be Ping, Http or Tcp.'
    }
    ...
    $check = @{
      ...
      SlowThresholdMs = Get-IntegerSetting `
        $entry `
        'SlowThresholdMs' 1000 1 120000
    }

    if ($entry.ContainsKey('Group')) {
      if (
        $entry.Group -isnot [string] -or
        [string]::IsNullOrWhiteSpace($entry.Group)
      ) {
        throw 'Group must be a non-empty string.'
      }
    ...
    if ($entry.Type -eq 'Http') {
      foreach ($key in @('HostName', 'Port')) {
        if ($entry.ContainsKey($key)) {
          throw "HTTP checks do not support $key."
        }
      }
      ...
      if (
        -not $entry.ContainsKey('Uri') -or
        $entry.Uri -isnot [string] -or
        -not [Uri]::TryCreate(
          $entry.Uri, [UriKind]::Absolute, [ref] $uri
        ) -or
        ...
        throw (
          'Uri must be an absolute HTTP(S) URL ' +
      ...
      if ($entry.ContainsKey('Method')) {
        if ($entry.Method -notin @('GET', 'POST')) {
      ...
      if ($entry.ContainsKey('Body')) {
        if ($entry.Body -isnot [string] -or $check.Method -ne 'POST') {
          throw (
            'Body must be a string and is only supported for POST.'
          )
        }
      ...
      if ($entry.ContainsKey('Headers')) {
        if ($entry.Headers -isnot [hashtable]) {
          throw (
            'Headers must be a hashtable.'
          )
        }
        ...
        foreach ($key in $entry.Headers.Keys) {
          if (
            $key -isnot [string] -or
            $key -notmatch '^[A-Za-z0-9-]+$' -or
            $entry.Headers[$key] -isnot [string] -or
            $entry.Headers[$key] -match '[\r\n]'
          ) {
            throw 'Invalid HTTP header name or value.'
          }

          $keyMatch = '(?i)authorization|cookie|token|api.?key|secret'
          if (
            $key -match $keyMatch -or
            $key -in @(
              'Host',
              'Content-Length',
              'Transfer-Encoding',
            ...
            throw (
              'Credentials and transport-controlled headers ' +
              'are not supported.'
            )
          }

          if (
            $key -like 'Content-*' -and
            (
              $key -ne 'Content-Type' -or
              $check.Method -ne 'POST'
            )
          ) {
            throw (
              'Only Content-Type on POST is supported ' +
              'as a content header.'
            )
          }
      ...
      if ($entry.ContainsKey('ExpectedStatusCodes')) {
        if (
          $entry.ExpectedStatusCodes -isnot [array] -or
          $entry.ExpectedStatusCodes.Count -eq 0
        ) {
          throw 'ExpectedStatusCodes must be a non-empty array.'
        }
        ...
        foreach ($code in $entry.ExpectedStatusCodes) {
          if (
            ...
            throw (
              'ExpectedStatusCodes must contain integers ' +
              'from 100 to 599.'
            )
          }
        }
      ...
      foreach ($key in $httpKeys) {
        if ($entry.ContainsKey($key)) {
          throw "Non-HTTP checks do not support $key."
        }
      }

      if (
        -not $entry.ContainsKey('HostName') -or
        $entry.HostName -isnot [string] -or
        [Uri]::CheckHostName($entry.HostName) -eq [UriHostNameType]::Unknown
      ) {
        throw 'HostName must be a valid host name or IP address.'
      }

      $check.HostName = $entry.HostName
      $check.Target = $entry.HostName
      if ($entry.Type -eq 'Tcp') {
        if (-not $entry.ContainsKey('Port')) {
          throw 'TCP checks require Port.'
        }

        $check.Port = Get-IntegerSetting $entry 'Port' 80 1 65535
        $check.Target = '{0}:{1}' -f $entry.HostName, $check.Port
      }
      elseif ($entry.ContainsKey('Port')) {
        throw 'Ping checks do not support Port.'
      }
    }
