...
.DESCRIPTION
Exercises parsing, configuration, localization, logging and scheduling with
temporary fixtures. Optional network cases use loopback only. Windows also
validates WPF loading. Throws on the first failure and removes temporary files
in finally.

.PARAMETER SkipNetwork
Omits loopback Ping, HTTP and TCP cases; scheduler and other non-network checks
still run.
...
function Assert-True {
  ...
  .DESCRIPTION
  Increments the script-scoped passed counter when the condition succeeds;
  otherwise throws with the test name.
...
function Assert-Rejected {
  ...
  .DESCRIPTION
  Writes a temporary fixture at script:testPath and passes it through the
  production importer. Any import exception counts as rejection.
  ...
  try {
    $null = Import-MonitorConfiguration $script:testPath
  }
  catch {
    $rejected = $true
  }
...
$temporary = Join-Path `
([IO.Path]::GetTempPath()) ('supervision-tests-' + [Guid]::NewGuid())
...
try {
  $children = Get-ChildItem -LiteralPath $root -Recurse -File `
  ...
  $configuration = Import-MonitorConfiguration `
  (Join-Path $root 'config/example.psd1')
